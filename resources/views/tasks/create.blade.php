...
<div class="max-w-7xl mx-auto space-y-8">
    <div>
        <h1 class="text-3xl font-bold text-gray-900 tracking-tight">إضافة مهمة جديدة</h1>
        <p class="text-sm text-gray-500 mt-1">أدخل تفاصيل المهمة وإسنادها إن لزم</p>
    </div>
                ...
                <div class="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" class="w-full px-3 py-3 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#1c5bb8] focus:ring-2 focus:ring-[#1c5bb8]/20 @error('title') border-red-500 @enderror">
                            @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">الوصف</label>
                            <textarea name="description" rows="4" class="w-full px-3 py-3 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#1c5bb8] focus:ring-2 focus:ring-[#1c5bb8]/20 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="case_id" class="w-full px-3 py-3 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#1c5bb8] focus:ring-2 focus:ring-[#1c5bb8]/20 @error('case_id') border-red-500 @enderror">
                                ...
                                @foreach($cases as $case)
                                    <option value="{{ $case->id }}" {{ (string) old('case_id') === (string) $case->id ? 'selected' : '' }}>{{ $case->case_number }} - {{ $case->title }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="lawyer_ids[]" multiple class="w-full px-3 py-3 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#1c5bb8] focus:ring-2 focus:ring-[#1c5bb8]/20 min-h-36 @error('lawyer_ids') border-red-500 @enderror @error('lawyer_ids.*') border-red-500 @enderror">
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ in_array($user->id, old('lawyer_ids', [])) ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date" value="{{ old('due_date') }}" class="w-full px-3 py-3 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#1c5bb8] focus:ring-2 focus:ring-[#1c5bb8]/20 @error('due_date') border-red-500 @enderror">
                            @error('due_date') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">وقت الاستحقاق</label>
                            <input type="time" name="due_time" value="{{ old('due_time') }}" class="w-full px-3 py-3 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#1c5bb8] focus:ring-2 focus:ring-[#1c5bb8]/20 @error('due_time') border-red-500 @enderror">
